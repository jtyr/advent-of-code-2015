...
little easier: find the surface area of the box, which is 2*l*w + 2*w*h +
...
def main():
    ...
    for line in f:
        ...
        surface = []
        surface.append(2*l*w)
        surface.append(2*w*h)
        surface.append(2*h*l)
        surface.append(sorted(surface)[0]/2)

        ribbon = sorted([l, w, h])

        total_surface += sum(surface)
        total_ribbon += (
            2*ribbon[0] + 2*ribbon[1] + reduce(lambda x, y: x*y, ribbon))

    print "[Star 1] Total surface: %d" % total_surface
    print "[Star 2] Total length opf ribbon: %d" % total_ribbon
