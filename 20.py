...
def sum_presents(house, n=10, limit=0):
    sum_house = 0

    for elf in xrange(1, house+1):
        tmp = float(house)/elf

        if tmp == int(tmp):
            if limit == 0 or limit > 0 and tmp <= limit:
                sum_house += elf * n
...
def main():
    ...
    while True:
        house_sum = sum_presents(house)

        if house_sum >= max_sum:
            break
        else:
            house += 1

    print "[Star 1] Lowest house number: %d" % house

    house = 705599

    while True:
        house_sum = sum_presents(house, 11, 50)
    ...
    print "[Star 2] Lowest house number: %d" % house
